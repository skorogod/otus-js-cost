...
export const getCategories = async (userId: string) => {
        ...
        if (user) {
                ...
                for(let category of Object.keys(categories)) {
                        let subCategories = await getDocs(query(collection(db, `users/${user.id}/categories/${category}/subCategories`)))
                        ...
                        if (subCategories) {
                                for (let subCat of subCategories.docs) {
                                        ...
                                        categories[category].subCategories[subCat.id] = {id: subCat.id, ...subCat.data()} as SubCategory
                                }
                        }
